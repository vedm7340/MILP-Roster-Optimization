...
for n in file_sizes
    ...
    @objective(model, Max, sum((ppg_w * players.PTS[p] + per_w * players.PER[p] + ws_w * players.WS[p] + threepoint_percent_w * players[!, "3P%"][p] + assists_w * players.AST[p] + games_w * players.G[p] + fouls_w * players.PF[p]) * selected[p] for p in 1:nrow(players)))
