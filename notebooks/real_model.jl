...
players_eFG_percent = Dict(df.Player .=> df[!, "eFG%"])
players_fouls = Dict(df.Player .=> df.PF)
players_turnovers = Dict(df.Player .=> df.TOV)
...
@objective(model, Max, sum((ppg_w * players_ppg[p] + per_w * players_per[p] + ws_w * players_ws[p] + games_w * players_games[p] + eFG_percent_w * players_eFG_percent[p] + fouls_w * players_fouls[p] + turnovers_w * players_turnovers[p]) * selected[p] for p in players))
...
if termination_status(model) != MOI.OPTIMAL
    ...
    println("Selected Players eFG%: ", [players_eFG_percent[p] for p in selected_players])
    println("Selected Players FPG: ", [players_fouls[p] for p in selected_players])
    println("Selected Players TPG: ", [players_turnovers[p] for p in selected_players])
    ...
    println("Average PPG: ", round(sum(players_ppg[p] for p in selected_players)/length(selected_players), digits=1))
    ...
    println("Average eFG%: ", round(sum(players_eFG_percent[p] for p in selected_players)/length(selected_players), digits=3))
    println("Average FPG: ", round(sum(players_fouls[p] for p in selected_players)/length(selected_players), digits=1))
    println("Average TPG: ", round(sum(players_turnovers[p] for p in selected_players)/length(selected_players), digits=1))
